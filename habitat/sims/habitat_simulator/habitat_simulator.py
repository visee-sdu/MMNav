...
# np.ndarray of shape (HEIGHT, WIDTH, 3), dtype=uint8，标准的 RGB 图像，每个像素值范围是 [0, 255]
...
# np.ndarray of shape (HEIGHT, WIDTH, 1), dtype=float32，每个像素代表深度，NORMALIZE_DEPTH=True，值范围是 [0, 1]（归一化）
@registry.register_sensor
...
# np.ndarray of shape (HEIGHT, WIDTH), dtype=uint32，每个像素值是语义标签的 ID（如“墙”是 1，“地板”是 2），不再是 RGB 图像，而是每个像素一个 class id
@registry.register_sensor
...
class HabitatSim(habitat_sim.Simulator, Simulator):
    ...
    def __init__(self, config: Config) -> None:
        ...
        agent_config = self._get_agent_config()  # AGENT_0的配置

        sim_sensors = []
        for sensor_name in agent_config.SENSORS:  # ['RGB_SENSOR', 'DEPTH_SENSOR', 'SEMANTIC_SENSOR']
    ...
    def reset(self) -> Observations:
        ...
        return [self._sensor_suite.get_observations(sim_obs[i]) for i in range(len(self.habitat_config.AGENTS))]  # List[Dict[str, Any]]，RGB, DEPTH, SENMENTIC
    ...
    def step(self, action: Union[str, np.ndarray, int, tuple]) -> Observations:
        ...
        return observations  # List[Dict[str, Any]]
    ...
    def geodesic_distance(
        ...
        if return_end_point:
            return path.geodesic_distance, path.points[-1]
        else:
    ...
    def _get_agent_config(self, agent_id: Optional[int] = None) -> Any:
        ...
        return agent_config  # AGENT_0的配置
