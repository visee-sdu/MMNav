...
class ObjectNavDatasetV1(PointNavDatasetV1):
    ...
    content_scenes_path: str = "{data_path}/content_samerotation400/{scene}.json.gz"
    ...
    def from_json(
    ...
    ) -> None:  # scenes_dir： /Data/hm3d/scene_datasets_v0.2/, json_str： /Data/hm3d/datasets/objectnav/objectnav_hm3d_v2/val/content/val_0000.json.gz
        deserialized = json.loads(json_str)
        if CONTENT_SCENES_PATH_FIELD in deserialized:  # "content_scenes_path"不在
        ...
        if "category_to_mp3d_category_id" in deserialized:  # 不在
            self.category_to_scene_annotation_category_id = deserialized[
        ...
        if "goals_by_category" not in deserialized:  # false
            deserialized = self.dedup_goals(deserialized)
        

        for k, v in deserialized["goals_by_category"].items():
            self.goals_by_category[k] = [self.__deserialize_goal(g) for g in v]  # goals_by_category[k][i]表示环境中某一个目标的信息，包含位置、视野等，goals_by_category[k][i].view_points[vidx]表示视野信息
            # self.object_position[k] = [g['accessible_point'] for g in v]

        for i, episode in enumerate(deserialized["episodes"]):
            episode = ObjectGoalNavEpisode(**episode)
            episode.id = episode.scene_id.split('/')[-1].split('.')[0] + '_'+ episode.episode_id
            ...
            # episode.goals = self.goals_by_category[episode.goals_key]  # 4ok3usBNeis.basis.glb_bed
            episode.goals = {self.category_to_task_category_id[c]: self.goals_by_category[k] for k, c in zip(episode.goals_key, episode.object_category)}  # 变成多目标
            # episode.object_positions = {self.category_to_task_category_id[c]: self.object_position[k] for k, c in zip(episode.goals_key, episode.object_category)}
