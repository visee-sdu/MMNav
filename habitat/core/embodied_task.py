...
class EmbodiedTask:
    ...
    def __init__(
        ...
        self.measurements = Measurements(
            self._init_entities(
                entity_names=config.MEASUREMENTS,  # ['DISTANCE_TO_GOAL', 'SUCCESS', 'SPL']
        ...
        self.sensor_suite = SensorSuite(
            self._init_entities(
                entity_names=config.SENSORS,  # ['OBJECTGOAL_SENSOR', 'GPS_SENSOR', 'COMPASS_SENSOR']
        ...
        self.actions = self._init_entities(
            entity_names=config.POSSIBLE_ACTIONS,  # ["STOP", "MOVE_FORWARD", "TURN_LEFT", "TURN_RIGHT", "LOOK_UP", "LOOK_DOWN"]
            register_func=registry.get_task_action,
            entities_config=self._config.ACTIONS,
        )
        self._action_keys = list(self.actions.keys())  # ["STOP", "MOVE_FORWARD", "TURN_LEFT", "TURN_RIGHT", "LOOK_UP", "LOOK_DOWN"]
    ...
    def assign_goals_to_agents(self, num_agents):
       
        if len(self.goals) == 0:
            raise ValueError("available_goals 不能为空")

        assignment = [-1]*num_agents  
        self.assigned_goals = {g: False for g in self.goals}  
        

        if num_agents <= len(self.goals):
            
            for i in range(num_agents):
                assignment[i] = self.goals[i]
                self.assigned_goals[self.goals[i]] = True
        else:
            
            for i in range(len(self.goals)):
                assignment[i] = self.goals[i]
                self.assigned_goals[self.goals[i]] = True
            for i in range(len(self.goals), num_agents):
                assignment[i] = random.choice(self.goals)
    ...
    def reset(self, episode: Episode):
        ...
        self.goal2position = {self._dataset.category_to_task_category_id[category]: None for category in episode.object_category}
        self.goal2agent = {self._dataset.category_to_task_category_id[category]: None for category in episode.object_category} 
        
        self.goals = [self._dataset.category_to_task_category_id[category] for category in episode.object_category]  
      
        self.assigned_goals = {g: False for g in self.goals}  
        self.goal_reassigned = [False] * len(self._sim.habitat_config.AGENTS)  
        
        
        self.agent2goal = self.assign_goals_to_agents(  # 初始每个智能体分配一个不同目标
            num_agents=len(self._sim.habitat_config.AGENTS))
        for i in range(len(self._sim.habitat_config.AGENTS)):
            observations[i]['objectgoal'] = np.array([self.agent2goal[i]])  # 分配给每个智能体一个目标
        
       
        
        self.goal_weight = {self._dataset.category_to_task_category_id[category]: 1 for category in episode.object_category}
        self.goal_priority = {self._dataset.category_to_task_category_id[category]: 0 for category in episode.object_category}  
        
      
        
        for i in range(len(self._sim.habitat_config.AGENTS)):
            print("The start position agent %d:" % (i), self._sim.get_agent_state(agent_id=i).position)
    ...
    def step(self, action: Dict[str, Any], episode: Episode):
        ...
        if isinstance(action_name, List):  # there are multiple actions
            ...
            for i in range(len(action_name)):
                if action_name[i] == 0:
                    if self.goal2position[self.agent2goal[i]] is None:
                        current_position = self._sim.get_agent_state(agent_id = i).position
                        self.goal2position[self.agent2goal[i]] = current_position  # 停止时记录目标位置
                        self.goal2agent[self.agent2goal[i]] = i  # 记录哪个智能体找到目标
                        print("Idx %d, agent %d found goal %d at position %s" % (self.idx, i, self.agent2goal[i], current_position))
                        
                        self.flag = True  # 标记有智能体找到目标,需要重分配目标
                       
                    else:
                        print("Idx %d, agent %d found goal %d at position %s, but it has been already found by agent %d" % (
                            self.idx, i, self.agent2goal[i], self.goal2position[self.agent2goal[i]], self.goal2agent[self.agent2goal[i]]
                        ))
            self.idx += 1
            ...
            self._is_episode_active = not all(v is not None for v in self.goal2position.values())
