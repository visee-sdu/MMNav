...
class ObjectGoalNavEpisode(NavigationEpisode):
    ...
    :param priority: priority of each category in the episode.
    :param weghts: Weights for each category in the episode.
    """
    ...
    # @property
    # def goals_key(self) -> str:  # 方法变为只读属性，可以像变量一样调用，不需要加括号
    #     r"""The key to retrieve the goals"""
    #     return f"{os.path.basename(self.scene_id)}_{self.object_category}"
    @property
    def goals_key(self) -> List[str]:
        r"""A list of keys to retrieve each goal separately"""
        return [
            f"{os.path.basename(self.scene_id)}_{cat}"
...
class ObjectGoalSensor(Sensor):
    ...
    def get_observation(
        ...
        **kwargs: Any,
        ...
        #     )
        #     return None
        category_name = episode.object_category  # 字符串
        if self.config.GOAL_SPEC == "TASK_CATEGORY_ID":  # GOAL_SPEC
            # return np.array(
            #     [self._dataset.category_to_task_category_id[category_name]],  # {"chair": 0, "bed": 1, "plant": 2, "toilet": 3, "tv_monitor": 4, "sofa": 5}
            #     dtype=np.int64,
            # )
            return np.array(
                [self._dataset.category_to_task_category_id[k] for k in category_name],
                dtype=np.int64,
            )
