...
from typing import Any, List, Optional, Tuple, Sequence, Union, Dict
...
def merge_sim_episode_config(sim_config: Config, episode: Episode, sim: Simulator) -> Any:
    ...
    if (
        ...
        #         agent_cfg.defrost()
        #         agent_cfg.START_POSITION = episode.start_position[0]
        #         agent_cfg.START_ROTATION = episode.start_rotation[0]
        ...
        for i in range(sim_config.NUM_AGENTS):
            ...
            agent_cfg = getattr(sim_config, agent_name)
            agent_cfg.defrost()
            agent_cfg.START_POSITION = episode.start_position[i]
            agent_cfg.START_ROTATION = episode.start_rotation[i]
            agent_cfg.IS_SET_START_STATE = True
            agent_cfg.freeze()
...
class NavigationGoal:
    ...
    # accessible_point: List[float] = attr.ib(default=None, validator=not_none_validator)
...
# 返回智能体的朝向（方向角，θ），相对于 episode 起始朝向。以弧度表示的 heading 角度，通常范围是 [-π, π]
...
class EpisodicCompassSensor(HeadingSensor):
    ...
    def get_observation(
        ...
        rotation_world_agent = agent_state.rotation  # 当前 agent 在世界坐标系下的朝向，类型是四元数。
        # rotation_world_start = quaternion_from_coeff(episode.start_rotation)
        # rotation_world_start = quaternion_from_coeff(episode.start_rotation[agent_id])
        rotation_world_start = quaternion_from_coeff(episode.start_rotation[0])  # episode 中 agent0（以agent0为参考系）初始位置的朝向，类型也是四元数。

        if isinstance(rotation_world_agent, quaternion.quaternion):
            return self._quat_to_xy_heading(
                rotation_world_agent.inverse() * rotation_world_start  # 当前 agent 朝向相对于初始agent0朝向的偏转。
            )
            # return self._quat_to_xy_heading(rotation_world_agent)
        else:
            raise ValueError("Agent's rotation was not a quaternion")
...
# 返回智能体当前位置（在 episode 起始坐标系下的相对位置）。 np.array([-agent_position[2], agent_position[0]])
...
class EpisodicGPSSensor(Sensor):
    ...
    def get_observation(
        ...
        # rotation_world_start = quaternion_from_coeff(episode.start_rotation)
        # origin = np.array(episode.start_position[agent_id], dtype=np.float32)
        origin = np.array(episode.start_position[0], dtype=np.float32)  # # 获取 agent0 的初始位置
        # rotation_world_start = quaternion_from_coeff(episode.start_rotation[agent_id])
        rotation_world_start = quaternion_from_coeff(episode.start_rotation[0])

        agent_position_world = agent_state.position
        agent_position_rel = agent_position_world - origin  #  # 世界坐标 → 相对坐标

        # agent_position = quaternion_rotate_vector(
        #     rotation_world_start.inverse(), agent_position - origin
        # )
        agent_position = quaternion_rotate_vector(
            rotation_world_start.inverse(), agent_position_rel  # # 把相对坐标旋转到 agent0 初始朝向的坐标系
        )
        if self._dimensionality == 2:
            return np.array(
                [-agent_position[2], agent_position[0]], dtype=np.float32  # 返回(-z, x)
            )
        else:
...
@registry.register_measure
class SPL(Measure):  # 只计算了0号智能体的SPL
...
class MultiAgentSPL(Measure):
    ...
    def reset_metric(self, episode, task, *args: Any, **kwargs: Any):
        ...
        self._episode_view_points = [
            ...
            ]  # 能够看到目标的视野位置， 目前一个episode只有一个目标

        for i in range(num_agents):
            pos = self._sim.get_agent_state(agent_id=i).position
            self._previous_position.append(pos)
            self._agent_episode_distance.append(0.0)

            self._start_end_episode_distance.append(
    ...
    def update_metric(self, episode, task: EmbodiedTask, *args: Any, **kwargs: Any):
        num_agents = self._sim.habitat_config.NUM_AGENTS
        success_distance_thresh = self._config.SUCCESS_DISTANCE
        ...
        for i in range(num_agents):
            ...
            # 自己判断是否成功（相对目标够近 + STOP）
            dist_to_goal = self._sim.geodesic_distance(
                current_position,
                self._episode_view_points,
                episode
            )
            per_agent_distance.append(dist_to_goal)

            is_success = (
                hasattr(task, "is_stop_called")
                and task.is_stop_called  # type: ignore
                and dist_to_goal < success_distance_thresh
            )
            per_agent_success.append(int(is_success))
            
            if is_success:
                ideal = self._start_end_episode_distance[i]
                actual = self._agent_episode_distance[i]
                spl = int(is_success) * (ideal / max(ideal, actual))
                per_agent_spl.append(spl)
            else:
                per_agent_spl.append(0.0)
        self._metric = {
            "spl_per_agent": per_agent_spl,
            "sr_per_agent": per_agent_success,
            "distance_per_agent": per_agent_distance,
            "max_spl": max(per_agent_spl),
            'max_sr': max(per_agent_success),
            'min_distance': min(per_agent_distance),
            'gt_distance': self._start_end_episode_distance,
            'agent_episode_distance': self._agent_episode_distance,
        }

@registry.register_measure
class MultiAgentMultiGoals(Measure):
    cls_uuid: str = "multi_agent_multi_goals"
    ...
    def reset_metric(self, episode, task, *args: Any, **kwargs: Any):
        self._previous_position = []  
        self._agent_episode_distance = []  
        self.goal_found = {k: 0 for k in episode.goals.keys()} 
        self._episode_view_points = {}  
        self.goal_dist = {k: None for k in episode.goals.keys()}  
        self._agent_episode_goal_distance = []  
        
        num_agents = self._sim.habitat_config.NUM_AGENTS
        self.initial_positions = []
        for i in range(num_agents):
            current_pos = self._sim.get_agent_state(agent_id=i).position
            self._previous_position.append(current_pos)
            self.initial_positions.append(current_pos.tolist())
            self._agent_episode_distance.append(0.0)
            self._agent_episode_goal_distance.append({k: 0.0 for k in episode.goals.keys()})
        
        for k in episode.goals.keys():
            self._episode_view_points[k] = [
                    view_point.agent_state.position
                    for goal in episode.goals[k]
                    for view_point in goal.view_points
                ]
        ...
        self.id = episode.id
        print('The episode is: ', self.id)
    
        self.ideal_dist = Distance.calculated_ideal_dist[self.id]
    ...
    def update_metric(self, episode, task: EmbodiedTask, *args: Any, **kwargs: Any):
        ...
        for i in range(num_agents):
            ...
            self._agent_episode_goal_distance[i][task.agent2goal[i]] += self._euclidean_distance(
                current_position, self._previous_position[i]
            )
            self._previous_position[i] = current_position
        
        for k in episode.goals.keys():
            prediction_postion = task.goal2position[k] 
            if prediction_postion is not None and self.goal_dist[k] is None:
                dist_to_goal = self._sim.geodesic_distance(
                    prediction_postion,
                    self._episode_view_points[k])
                self.goal_dist[k] = dist_to_goal  
                if dist_to_goal < self._config.SUCCESS_DISTANCE:
                    self.goal_found[k] = 1.0

        num_goals = len(self.goal_found)
        num_found = sum(self.goal_found.values())
        
        asr = 1.0 if num_found > 0 else 0.0
        avgsr = num_found / num_goals
        tsr = 1.0 if num_found == num_goals else 0.0
        tspl = 0.0
        tspl_max = 0.0
        if tsr == 1.0:
           
                
            tspl = self.ideal_dist/max(self.ideal_dist, sum(self._agent_episode_distance))
            
            actual = 0.0
            for g in task.goal2agent.keys():
                a = task.goal2agent[g]
                actual += self._agent_episode_goal_distance[a][g]
            tspl_max = self.ideal_dist/max(self.ideal_dist, actual)
        ...
        self._metric = {
            ...
            'ideal_dist': self.ideal_dist, 
            'agent_episode_distance': self._agent_episode_distance, 
            'agent_episode_goal_distance': self._agent_episode_goal_distance,  #
            "asr": asr, 
            'avgsr': avgsr,  
            'tsr': tsr, 
            'tspl': tspl,  
            'tspl_max': tspl_max,  
            'goal_agent': task.goal2agent, 
        }
