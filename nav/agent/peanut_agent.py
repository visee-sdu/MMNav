...
class PEANUT_Agent(habitat.Agent):
    def __init__(self, args, task_config: habitat.Config, agent_id=0):
        self.agent_id = agent_id
        self._POSSIBLE_ACTIONS = task_config.TASK.POSSIBLE_ACTIONS
        self.agent_states = Agent_State(args)
        self.agent_helper = Agent_Helper(args, self.agent_states, agent_id)
    ...
    def reset(self, obs):
        ...
        self.agent_states.reset(self.get_sim_location(obs))
        self.first_obs = True
        self.step = 0
        self.timestep = 0
    ...
    def mapping(self, observations):
        # Preprocess observations
        # assert len(observations['objectgoal']) == 1, 'Only one goal is supported in PEANUT'
        goal = observations['objectgoal'][0]  
        info = self.get_info(observations) 
        if self.args.use_gt_seg:
            info['goalseg'] = observations['goalseg']
        ...
        if self.first_obs:
            self.agent_states.init_with_obs(obs, info)
            self.first_obs = False
        self.agent_states.mapping(obs, info)
        return info
    ...
    def act2(self, infos):
        ...
        if self.timestep > self.args.timestep_limit:
            return {'action': 0}
        planner_inputs = self.agent_states.predict(infos)
        action = self.agent_helper.plan_act(planner_inputs, self.agent_id)
