...
from constants import coco_to_hm3d
from refine import UAGRM, UAGRM_Large, UAGRM_noconf
...
class Agent_State:
    ...
    def __init__(self,args):
        ...
        self.nc = 4 + args.num_sem_categories  # num channels in map, 4+10
        
        
        self.map_center_x, self.map_center_y = args.map_size_cm/100.0/2.0, args.map_size_cm/100.0/2.0  
        ...
        self.uagrm = UAGRM_noconf().to(self.device)  
        state_dict = torch.load(
            './uagrm/uagrm.pth',
            map_location=self.device
        )
        self.uagrm.load_state_dict(state_dict)
        
        total_1 = sum(p.numel() for p in self.sem_map_module.parameters())
        total_2 = sum(p.numel() for p in self.prediction_model.model.parameters())
        total_3 = sum(p.numel() for p in self.uagrm.parameters())
        print("Total parameters:", total_1 + total_2 + total_3)
        print("Trainable parameters:", total_1 + total_2 + total_3)

    
    def reset(self, init_pose):
        """Reset per episode."""
        if Agent_State.global_map is None:
            Agent_State.global_map = torch.zeros(self.nc, self.full_w, self.full_h).float().to(self.device)
        if Agent_State.origin_pose is None:
            Agent_State.origin_pose = np.array([init_pose[0], init_pose[1], init_pose[2].item()])  
            print("Agent_State.origin_pose:", Agent_State.origin_pose)
        self.l_step = 0
        self.step = 0
        self.goal_cat = -1
        self.previous_goal_cat = -1
    ...
    def get_local_map_boundaries(self, agent_loc, local_sizes, full_sizes):
        ...
        local_w, local_h = local_sizes  # 480
        full_w, full_h = full_sizes  # 960
    ...
    def init_map_and_pose(self):
        ...
        g_pose = self.convert_global_pose(self.init_pose[0], self.init_pose[1])  
        self.full_pose[0] = g_pose[0]  # x
        self.full_pose[1] = g_pose[1]  # y, 
        
        

        locs = self.full_pose.cpu().numpy()  # （24，24， 0）
    ...
    def world2map(self, x_world, y_world, map_resolution, map_center_x, map_center_y):
        x = int(x_world * 100.0 / map_resolution) + map_center_x
        y = int(y_world * 100.0 / map_resolution) + map_center_y
        return x, y
    
    def world2map(self, x_world, y_world, map_resolution, origin_pose, map_center_x, map_center_y):
        dx = x_world - origin_pose[0]
        dy = y_world - origin_pose[1]
        x_pixel = int(dx * 100.0 / map_resolution) + map_center_x
        y_pixel = int(dy * 100.0 / map_resolution) + map_center_y
        return x_pixel, y_pixel
    
    def convert_global_pose(self, x_world, y_world):
        dx = x_world - Agent_State.origin_pose[0]
        dy = y_world - Agent_State.origin_pose[1]
        x = dx + self.map_center_x
        y = dy + self.map_center_y
    ...
    def mapping(self, obs, infos):
        ...
        if self.l_step == args.num_local_steps - 1 or self.previous_goal_cat != self.goal_cat:  # num_local_steps=20
            ...
            # If we don't want to activate prediction yet
            if self.step < args.switch_step:  # switch_step=0
                # Set goal to corner (like Stubborn)
                preset = self.global_goal_presets[self.global_goal_preset_id]
                self.global_goals = [[int(preset[0] * self.local_w), int(preset[1] * self.local_h)]]
                self.global_goals = [[min(x, int(self.local_w - 1)),
    ...
    def predict(self, infos):
        ...
        if (self.step % args.update_goal_freq == args.update_goal_freq - 1 or 
            self.step == 0 or
            self.dist_to_goal < args.goal_reached_dist or self.previous_goal_cat != self.goal_cat) and self.step >= args.switch_step:
            
            self.update_prediction()  
            self.update_global_goal() 
            
            if self.previous_goal_cat != self.goal_cat:
                self.previous_goal_cat = self.goal_cat
                print('Updating goal category to:', coco_to_hm3d[self.goal_cat])
    ...
    def update_global_map(self):
        Agent_State.global_map[:, self.lmb[0]:self.lmb[1], self.lmb[2]:self.lmb[3]] = \
            torch.max(Agent_State.global_map[:, self.lmb[0]:self.lmb[1], self.lmb[2]:self.lmb[3]],
                      self.local_map)  
        self.full_map = Agent_State.global_map
        self.full_map[2:4, self.lmb[0]:self.lmb[1], self.lmb[2]:self.lmb[3]] = self.local_map[2:4, :, :]  
    ...
    def update_prediction(self):
        ...
        assert self.full_w != args.prediction_window    
        ...
        if self.full_w == args.prediction_window and self.full_h == args.prediction_window:  
            ...
            object_preds = self.refine_prediction(object_preds, self.step)
        else:
            ...
            temp = np.full((object_preds.shape[0], self.full_w, self.full_h), -100000.0)
            temp[:, x1:x2, y1:y2] = object_preds
            object_preds = temp  
            self.probability_preds = temp * (self.full_map[1].cpu().numpy()<0.5)
            self.probability_preds = np.exp(self.probability_preds/10.0) 
    ...
    def refine_prediction(self, object_preds, t):
        # y = object_preds
        time = torch.LongTensor([t//25]).to(self.device)
        object_preds = torch.from_numpy(object_preds).float().to(self.device)  
        object_preds = object_preds.unsqueeze(0)  # [1, C, H, W]
        return self.uagrm(object_preds, time).squeeze(0).cpu().numpy()  # [C, H, W]
    ...
    def update_global_goal(self):
        ...
        temperature = args.dist_weight_temperature / args.map_resolution  # 500/5
        self.dist_wt = np.exp(-dd / temperature)  
        dd_wt = np.exp(-dd / temperature)[self.lmb[0]:self.lmb[1], self.lmb[2]:self.lmb[3]]  
        
        if np.sum(dd_wt) < 10 and self.dd_wt is not None: 
            dd_wt = self.dd_wt
        if args.dist_weight_temperature == -1: 
            value = self.target_pred
        elif args.dist_weight_temperature == 0: 
        ...
        if new_global_goal != self.last_global_goal:  
