...
class SemanticPredMaskRCNN():
    ...
    def __init__(self, args):
        ...
        cfg.MODEL.ROI_HEADS.SCORE_THRESH_TEST = args.sem_pred_prob_thr  # 0.95
        cfg.MODEL.WEIGHTS = args.seg_model_wts  # nav/agent/utils/mask_rcnn_R_101_cat9.pth
        cfg.MODEL.DEVICE = args.sem_gpu_id  # 0
        
        self.n_cats = cfg.MODEL.ROI_HEADS.NUM_CLASSES  # 9
    ...
    def get_prediction(self, img, depth=None, goal_cat=None):
        ...
        img = img[:, :, ::-1]  # 将图像的通道顺序从 RGB 变成 BGR。
        pred_instances = self.predictor(img)["instances"]
        
        semantic_input = torch.zeros(img.shape[0], img.shape[1], self.n_cats + 1, device=args.sem_gpu_id)  # (H, W, 10), 10个类别+1个背景
        for j, class_idx in enumerate(pred_instances.pred_classes.cpu().numpy()):
            if class_idx in range(self.n_cats):
                ...
                if (confscore < args.sem_pred_prob_thr): 
                    continue
                if idx == goal_cat:
                    if confscore < args.goal_thr:  # 0.985
                        continue
                obj_mask = pred_instances.pred_masks[j] * 1.
                semantic_input[:, :, idx] += obj_mask
        
        return semantic_input.cpu().numpy(), img  # (H, W, 10)
